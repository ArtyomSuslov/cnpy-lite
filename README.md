...
NumPy offers the `save` method for easy saving of arrays into `.npy` files.

`cnpy` lets you read and write `.npy` files in C++.
...
There is one function for writing data: `npy_save`.

There is one function for reading: `npy_load`, which will load a .npy file.
